...
The `--untagged-highlight` CSS variable provides the untagged highlight color (#e5e7eb light, #4a4a6a dark) — Plan 04's HighlightRenderer update will use `var(--untagged-highlight)` instead of hardcoded `#e5e7eb`.
  </action>
  <verify>
    <automated>cd "/Users/bharadwajkulkarni/Documents /Bawa's Lab/mckenna-wiki" && grep -c "@custom-variant dark" src/app/globals.css && grep -c "highlight-opacity" src/app/globals.css && grep -c "untagged-highlight" src/app/globals.css</automated>
    <manual>Verify CSS contains both :root and .dark variable blocks with the correct palette values, including --untagged-highlight</manual>
  </verify>
  <done>globals.css has @custom-variant dark, light/dark CSS variable palettes, highlight-opacity variable, and untagged-highlight variable</done>
</task>
...
**Toggle positioning note:** User decision says "sun/moon icon in header/navbar." However, the app has no shared navbar component — each page has its own inline header (see RESEARCH.md "Navbar/Header" section). Creating a shared NavBar would be a structural refactor beyond this plan's scope. Instead, the toggle uses `fixed top-4 right-4 z-50` positioning to float above all page content, providing the same UX intent (always-visible toggle) without conflicting with existing page layouts. The `z-50` ensures it appears above sidebars (which use z-10/z-20). A shared NavBar can be added in a future phase if desired.
