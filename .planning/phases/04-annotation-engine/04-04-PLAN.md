...
1. Add new optional prop for highlights:
```typescript
...
2. Import the highlight renderer:
```typescript
import { renderTextWithHighlights } from '@/components/annotations/HighlightRenderer'
import type { ParagraphHighlight } from '@/lib/types/annotation'
```

3. Update the text rendering logic:
```typescript
{/* Paragraph text with search highlighting and/or annotation highlights */}
<p
...
>
  {searchQuery ? (
    // Search highlighting takes priority (uses Highlighter component)
    <Highlighter
...
- Search highlighting takes priority over annotation highlights (they serve different purposes)
- Click handler checks for data-annotation-id and calls callback
  </action>
  <verify>`npm run build` passes. ParagraphView renders both with and without highlights prop.</verify>
  <done>ParagraphView displays highlights when provided, handles clicks on highlighted text.</done>
</task>
