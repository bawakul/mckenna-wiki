...
```typescript
// Container ref for selection detection - shared with virtualizer
...
4. **CRITICAL: Implement callback ref for sharing between virtualizer and selection.**

The virtualizer uses `parentRef` for scroll container. Selection detection uses `containerRef`. Both need the same DOM element. Use a callback ref pattern:

```typescript
// Callback ref that assigns to both refs
const sharedRef = useCallback((el: HTMLDivElement | null) => {
  // parentRef comes from useVirtualizer (already a RefObject)
  // We need to use the mutable .current property
...
5. Update container div to use the shared ref:
```typescript
return (
  <div
    ref={sharedRef}  // Use callback ref, NOT inline function
...
**Why callback ref instead of inline function:**
- Inline refs like `ref={(el) => { ... }}` create new functions on every render
- This causes React to call the ref with null, then the new element on each render
- Callback refs with useCallback are stable across renders
- Both parentRef (virtualizer) and containerRef (selection) need the same element
  </action>
...
- [ ] Callback ref correctly shares element between virtualizer and selection
